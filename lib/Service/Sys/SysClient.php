...
class SysClient
{
    ...
    /**
     * @param Model\TradeDividequeryRequest $request
     * @return Model\TradeDividequeryResponse
     * @throws YopClientException
     */
    public function tradeDividequery(Model\TradeDividequeryRequest $request)
    {
        if ($request == null) {
            throw new YopClientException("request is required.");
        }
        $clientExecutionParams = new ClientExecutionParams($request, Model\TradeDividequeryRequestMarshaller::getInstance(),
            Model\TradeDividequeryResponseUnMarshaller::getInstance());
        return $this->clientHandler->execute($clientExecutionParams);
    }
    /**
     * @param Model\TradeFullsettleRequest $request
     * @return Model\TradeFullsettleResponse
     * @throws YopClientException
     */
    public function tradeFullsettle(Model\TradeFullsettleRequest $request)
    {
        if ($request == null) {
            throw new YopClientException("request is required.");
        }
        $clientExecutionParams = new ClientExecutionParams($request, Model\TradeFullsettleRequestMarshaller::getInstance(),
            Model\TradeFullsettleResponseUnMarshaller::getInstance());
        return $this->clientHandler->execute($clientExecutionParams);
    }
    /**
